...
import core.dto.JavaMockMethodInfoDTO;
import core.dto.JavaParameterDTO;
import core.model.JavaClassModel;
import core.model.JavaGenInfoModel;
import core.model.JavaMethodModel;
import core.model.JavaParameteModel;
...
public class BuildMockClassMethod {
     ...
     * @param m                         正则匹配
     */
    private static void saveMockMethodInfoDTO(JavaGenInfoModel javaGenInfoModel, List<JavaMockMethodInfoDTO> javaMockMethodInfoDTOList,
        ...
        if (javaClassModel == null) {
            log.warn("获取的mock类数据为NULL，" + "mockJavaClassModelMap=" + javaGenInfoModel + ",name=" + name + ",方法源码=" + methodCode);
            return;
        }
...
//        }
     ...
     * @return 方法信息
     */
    private static JavaMethodModel getJavaMethodModelByParent(JavaGenInfoModel javaGenInfoModel, String name, String methodName,
                                                              JavaMethodModel javaMethodModel) {
        //通过父类再进行获取
        JavaClass javaClass = BaseConstant.javaProjectBuilder.getClassByName(name);
        if (javaClass == null) {
            log.warn("没有找到该类，类名：" + name + "，javaClass=null");
            return null;
        }
        JavaClass superJavaClass = javaClass.getSuperJavaClass();
        if (superJavaClass == null) {
            log.warn("没有找到该类的父类，类名：" + name + "，superJavaClass=null，javaClass=" + javaClass);
            return null;
        }
        ...
        if (javaClassModel1 == null) {
            log.warn("没有找到该父类的JavaClassModel，superJavaClass：" + superJavaClass + "，javaClass：" + javaClass + "，javaGenInfoModel=" + javaGenInfoModel);
            return null;
        }
        ...
        log.warn("在类中没有找到该方法，方法名：" + methodName + "，类名：" + name + "，javaGenInfoModel=" + javaGenInfoModel);
